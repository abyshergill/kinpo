    ...
    // Master Color Database Bank Matrix
    const masterColors = [
        { name: "Red", hex: "#e74c3c" },   { name: "Blue", hex: "#3498db" },
        { name: "Green", hex: "#2ecc71" },  { name: "Yellow", hex: "#f1c40f" },
        { name: "Orange", hex: "#e67e22" }, { name: "Purple", hex: "#9b59b6" },
        { name: "Pink", hex: "#ff8f8f" },   { name: "Brown", hex: "#784212" },
        { name: "Black", hex: "#2c3e50" },  { name: "White", hex: "#ffffff" }
    ];
    ...
    let score = 0;
    let targetQuestionColor = null;
    let activePool = [...masterColors];
    ...
    const typeColorFlash = document.getElementById('type-color-flash');
    ...
    function buildParentChecklist() {
        ...
        masterColors.forEach((color) => {
            ...
            check.type = "checkbox";
            check.checked = true; 
            check.value = color.name;
            
            check.addEventListener('change', () => {
                SoundFX.pop();
                rebuildActiveColorsPool();
            });
    ...
    function rebuildActiveColorsPool() {
        ...
        activePool = masterColors.filter(c => checkedNames.includes(c.name));
        
        if (activePool.length === 0) {
            activePool = [masterColors[0]]; 
    ...
    function nextChallengeCycle() {
        ...
        targetQuestionColor = activePool[Math.floor(Math.random() * activePool.length)];

        if (currentMode === "type") {
            typeColorFlash.style.backgroundColor = targetQuestionColor.hex;
            if(parentDashboard.style.display === 'none') playerTypeInput.focus();
        } else {
            pickTextPrompt.innerText = targetQuestionColor.name;
            
            let choices = [targetQuestionColor];
            let distractorPool = masterColors.filter(c => c.name !== targetQuestionColor.name);
            
            while (choices.length < Math.min(4, masterColors.length)) {
                let randColor = distractorPool[Math.floor(Math.random() * distractorPool.length)];
                if(!choices.includes(randColor)) {
                    choices.push(randColor);
                }
            }
            ...
            choices.forEach(color => {
                ...
                circle.classList.add('color-choice-swatch');
                circle.style.backgroundColor = color.hex;
                circle.setAttribute('data-color-name', color.name);
                
                circle.addEventListener('click', function() {
                    evaluatePickChoice(this.getAttribute('data-color-name'));
                });
    ...
    btnCheckType.addEventListener('click', () => {
        ...
        let correctAnswer = targetQuestionColor.name.toLowerCase();
    ...
    function evaluatePickChoice(clickedName) {
        if (clickedName === targetQuestionColor.name) {
            triggerSuccessCelebration();
        } else {
            triggerRetryNotification();
        }
    ...
    function triggerSuccessCelebration() {
        ...
        modalDesc.innerText = `Awesome job recognizing colors, ${childName}!`;
    ...
    function triggerRetryNotification() {
        ...
        modalDesc.innerText = currentMode === "type" 
            ...
            : "Look closely at the word, and pick a different color option!";
        celebrateModal.style.display = "flex";
    }
